...
#os.chdir('P:\GitHub_GitLab\ClosedLoop\Scripts')
...
#    d['LOBO_stable_face_acc_corr'] = face_acc
    ...
    offset_pred_lst = []
    runwise_accuracies = [] # save runwise accuracies
    c_test = 0
    ...
    for r in range(n_it): # 5 runs
        ...
        # Save the accuracy per run for computing the standard error of the mean.
        above_chance_run = len(np.where((np.array(alpha_test[c_test-200:c_test])>0.5))[0])/len(alpha_test[c_test-200:c_test])
        
        runwise_accuracies.append(above_chance_run)
    ...
    score = metrics.accuracy_score(y_stable_blocks[400:], y_pred[:1000]) 
    ...
    d['LORO_stable_acc_runwise'] = runwise_accuracies
    d['LORO_stable_std_runwise'] = np.std(runwise_accuracies)
    
    CI66_plus = above_chance_train + ((np.std(runwise_accuracies)))
    CI66_minus = above_chance_train - ((np.std(runwise_accuracies)))
    
    d['LORO_CI66_plus'] = CI66_plus
    d['LORO_CI66_minus'] = CI66_minus
    
    # Check for significance
    CI66_significance = False
    if CI66_minus > 0.5:
        CI66_significance = True
    
    d['LORO_CI66_significant'] = CI66_significance
    
    # Compute the 95% CI by adding and subtracting 1.96 standard deviations 
    
    CI95_plus = above_chance_train + (1.96 * (np.std(runwise_accuracies)))
    CI95_minus = above_chance_train - (1.96 * (np.std(runwise_accuracies)))
    
    d['LORO_CI95_plus'] = CI95_plus
    d['LORO_CI95_minus'] = CI95_minus
    
    # Check for significance
    CI95_significance = False
    if CI95_minus > 0.5:
        CI95_significance = True
    
    d['LORO_CI95_significant'] = CI95_significance
...
#        #mne.viz.plot_compare_evokeds(e_dict,ci=0.95,picks=[7],colors=colors)
    ...
    # PICKLE TIME
    fname = '15Oct_subj_'+str(subjID)+'.pkl'
