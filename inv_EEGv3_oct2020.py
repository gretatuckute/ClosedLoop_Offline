# -*- coding: utf-8 -*-
"""
Investigate trends across sessions according to reviewer #2.
...
scriptsDir = '/Users/gt/pCloud Drive/Previous/GitHub_GitLab/GitHub_from_Dell_09072020/ClosedLoop_Offline/test_aug2020/'
os.chdir(scriptsDir)

#%% MANUAL VARIABLES OF INTEREST
...
npyDir = '/Users/gt/pCloud Drive/Previous/GitHub_GitLab/ClosedLoop_October2019/Documents/npy/'
...
# d_all_stats, oct15, has CI stats, but not RT per run acc
...
#%% ######## FUNCTIONS ########
def extractVal(wanted_key, dictname=d_all3):
    '''
...
subsAll_ssp, subsNF_ssp, subsC_ssp, meanAll_ssp, meanNF_ssp, meanC_ssp = extractVal('RT_test_acc_corr', d_ssp)
subsAll_no, subsNF_no, subsC_no, meanAll_no, meanNF_no, meanC_no = extractVal('RT_test_acc_corr', d_no)

stats.ttest_ind(subsAll_ssp,subsAll_no)
stats.ttest_rel(subsAll_ssp,subsAll_no)

stats.ttest_ind(subsNF_ssp,subsNF_no)
stats.ttest_rel(subsNF_ssp,subsNF_no)

stats.ttest_ind(subsC_ssp,subsC_no)
stats.ttest_rel(subsC_ssp,subsC_no)
...
subsC_sorted = [y for x,y in sorted(zip(idxLst,subsC))] # I.e. just sort behavioral measure based on this list. 
subsC_sorted = np.asarray(subsC_sorted)
...
for subj in np.arange(1, len(subsNF_run)):
    slope, intercept, r_value, p_value, std_err = stats.linregress(np.arange(1,6), subsNF_run[subj])
    print(slope, p_value)

for subj in np.arange(1, len(subsNF_run)):
    slope, intercept, r_value, p_value, std_err = stats.linregress(np.arange(1,6), subsC_run[subj])
    print(slope, p_value)

# Compute run 1 versus run 5
all_run1 = []
all_run5 = []
for run in subsAll_run:
    all_run1.append(run[0])
    all_run5.append(run[4])

stats.ttest_rel(all_run1,all_run5)
stats.ttest_ind(all_run1,all_run5)

NF_run1 = []
NF_run5 = []
for run in subsNF_run:
    NF_run1.append(run[0])
    NF_run5.append(run[4])

stats.ttest_rel(NF_run1,NF_run5)
# stats.ttest_ind(NF_run1,NF_run5)

C_run1 = []
C_run5 = []
for run in subsC_run:
    C_run1.append(run[0])
    C_run5.append(run[4])

stats.ttest_rel(C_run1,C_run5)
# stats.ttest_ind(C_run1,C_run5)

# Test interaction
fvalue, pvalue = stats.f_oneway(NF_run1, NF_run5, C_run1, C_run5)

# INTERACTION
# Find diffs
NF_diff = np.array(NF_run5) - np.array(NF_run1)
C_diff = np.array(C_run5) - np.array(C_run1)
